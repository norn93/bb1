...
def test_diagnostic(stat):
    ...
    if time_to_launch < 10:
        ...
        stat.summary(diagnostic_msgs.msg.DiagnosticStatus.ERROR,
            "Launch in %f seconds..." % time_to_launch)
    else:
        # summary for unformatted text. It's just the same ;)
        stat.summary(diagnostic_msgs.msg.DiagnosticStatus.OK,
            "Launch is in a long time.")
...
class TestClass:
    def produce_diagnostics(self, stat):
        stat.summary(diagnostic_msgs.msg.DiagnosticStatus.WARN, "This is an updater.")
        stat.add("Updater stat", 1000.)
        return stat

class TestTask(diagnostic_updater.DiagnosticTask):
    def __init__(self):
...
if __name__=='__main__':
    ...
    updater.add("Function updater", test_diagnostic)
    dc = TestClass()
    ...
    freq_bounds = {'min':0.5, 'max':20} # If you update these values, the
